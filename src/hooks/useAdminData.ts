...
export function useCreateAdminCheckout(
  userId: string | null,
  hostelIds: string[]
) {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: (payload: {
      plan_id: string;
      hostel_id: string;
    }) => createAdminOrder(userId!, hostelIds, payload),

    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: ["admin-subscription", userId, hostelIds],
      });
...
export function useVerifyAdminPayment(
  userId: string | null,
  hostelIds: string[]
) {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: (payload: VerifyPaymentPayload) =>
      verifyAdminPayment(userId!, hostelIds, payload),
