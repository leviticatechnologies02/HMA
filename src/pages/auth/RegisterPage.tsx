...
import { Lock, Phone, User, ArrowRight, CheckCircle, XCircle, Eye, EyeOff } from "lucide-react";
...
function PasswordStrength({ password }: { password: string }) {
  ...
  const strength = score <= 1 ? { label: "Weak", color: "bg-error" } : score === 2 ? { label: "Fair", color: "bg-warning" } : score === 3 ? { label: "Good", color: "bg-secondary" } : { label: "Strong", color: "bg-success" };
    ...
    <div className="mt-2 space-y-2">
      <div className="flex items-center gap-2">
        <div className="flex-1 h-1.5 bg-slate-200 rounded-full overflow-hidden">
          <div className={`h-full rounded-full transition-all duration-300 ${strength.color}`} style={{ width: `${(score / 4) * 100}%` }} />
        </div>
        <span className={`text-xs font-semibold ${score <= 1 ? "text-error" : score === 2 ? "text-warning" : score === 3 ? "text-secondary" : "text-success"}`}>{strength.label}</span>
      </div>
      <div className="grid grid-cols-2 gap-1">
        {checks.map(c => (
          <div key={c.label} className={`flex items-center gap-1 text-xs ${c.ok ? "text-success" : "text-slate-400"}`}>
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 dark:bg-[#1A1A2E] dark:border-[#2D2D4A]">
          <div className="text-center mb-8">
            ...
            <p className="mt-2 text-slate-600 dark:text-[#B0B8C8]">Join StayEase to find your perfect hostel</p>
          </div>
          ...
          <form className="space-y-4" onSubmit={onSubmit}>
            ...
            <div>
              ...
              {errors.retype_password && (
                <p className="mt-1 text-sm text-error">{errors.retype_password.message}</p>
              )}
            </div>
