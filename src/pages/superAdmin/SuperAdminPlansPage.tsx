...
export function SuperAdminPlansPage() {
  ...
  const { data, isLoading, isError } = useSuperAdminPlans(
    userId,
    {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
        <div>
          ...
          <p className="mt-1 text-slate-500">Create and manage subscription plans for hostels.</p>
        </div>
        <button onClick={handleNewPlan} className="btn-primary flex items-center justify-center gap-2 w-full sm:w-auto text-sm">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          { label: "Total Plans", value: plans.length, icon: <Package className="w-5 h-5" />, color: "bg-primary/10 text-primary" },
          { label: "Active", value: active, icon: <CheckCircle className="w-5 h-5" />, color: "bg-success/10 text-success" },
          { label: "Inactive", value: plans.length - active, icon: <XCircle className="w-5 h-5" />, color: "bg-error/10 text-error" },
        ].map((s) => (
      ...
      <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
        ...
        {isError && <div className="p-8 text-center text-error">Failed to load plans.</div>}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50">
                <tr>
                  {["Plan Name", "Code", "Price/Month", "Duration", "Hostels", "Status", "Created", "Actions"].map((h) => (
              ...
              <tbody>
                ...
                {filteredPlans.map((p) => (
                  <tr key={p.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                    <td className="px-5 py-4 font-medium text-dark">{p.name}</td>
                    <td className="px-5 py-4 text-slate-600">{p.code}</td>
                    <td className="px-5 py-4 font-semibold text-primary">₹{p.price_monthly.toLocaleString()}</td>
                    <td className="px-5 py-4 text-slate-600">{p.duration_days} days</td>
                    <td className="px-5 py-4 text-center text-slate-600">{p.hostel_limit}</td>
                    <td className="px-5 py-4">
                      <span className={`badge ${STATUS_BADGE[p.status] || "badge-slate"} text-xs`}>
                        {p.status}
                      </span>
                    </td>
