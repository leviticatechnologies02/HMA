...
export function AdminSettingsPage() {
  ...
  const { data: profile, isLoading, isError } = useAdminProfile(userId, hostelIds);
  ...
  const passwordFormik = useFormik({
    ...
    onSubmit: async (values, { setFieldError, resetForm }) => {
      try {
        // Change password directly (API will validate the current password)
  ...
  if (isError || !profile) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-center">
        <h2 className="text-red-700 font-bold text-lg mb-2">Failed to load profile</h2>
        <p className="text-red-600 text-sm mb-4">There was an error fetching your admin profile data.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-3xl border border-slate-100 p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-5">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-2xl sm:text-3xl font-heading font-bold text-dark">{profile.full_name}</h1>
          <p className="text-sm text-slate-500 mt-1 capitalize">{profile.role?.replace(/_/g, " ")}</p>
          <div className="mt-2 flex items-center gap-2 flex-wrap">
            <span className="badge badge-primary capitalize text-xs">Admin</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        <div className="bg-white rounded-2xl border border-slate-100 p-5">
          ...
          <p className="font-medium text-sm text-dark break-all">{profile.email}</p>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-5">
          ...
          <p className="font-medium text-sm text-dark">{profile.created_at ? formatDate(profile.created_at) : "N/A"}</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-5">
          ...
          <p className="font-medium text-sm text-dark">{profile.updated_at ? formatDate(profile.updated_at) : "N/A"}</p>
        </div>
      </div>
