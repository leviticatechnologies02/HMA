...
export function AdminInventoryPage() {
  ...
  const createRoomMutation = useCreateAdminRoom(
    userId,
    selectedHostelId,
    hostelIds,
  );
  const createBedMutation = useCreateAdminBed(
    userId,
    selectedRoomId,
    hostelIds,
  );
  const updateRoomMutation = useUpdateAdminRoom(
    userId,
    selectedHostelId,
    hostelIds,
  );
  const updateBedMutation = useUpdateAdminBed(
    userId,
    selectedRoomId,
    hostelIds,
  );
  const deleteRoomMutation = useDeleteAdminRoom(
    userId,
    selectedHostelId,
    hostelIds,
  );
  ...
  const [editRoomForm, setEditRoomForm] = useState<
    Partial<typeof defaultRoomForm>
  >({});
    ...
    onSubmit: (values, { resetForm }) => {
      createRoomMutation.mutate(
        ...
        },
        {
          ...
          onError: (error: any) => {
            const errorMessage =
              error?.response?.data?.detail || "Failed to create room";
  ...
  const bedFormik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      createBedMutation.mutate(values, {
        ...
        onError: (error: any) => {
          const errorMessage =
            error?.response?.data?.detail || "Failed to create bed";
          toast.error(errorMessage);
        },
      });
    },
  ...
  if (!userId || !hostelIds.length) {
    return (
      <div>
        Please login as an admin with assigned hostels to manage inventory.
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400">
          Create and manage rooms and beds for your hostel.
        </p>
      </div>
      ...
      <section className="rounded-2xl bg-white dark:bg-slate-900 p-4 sm:p-5 md:p-6 shadow-sm dark:shadow-md border border-slate-100 dark:border-slate-800">
        <label className="mb-2 block text-xs sm:text-sm font-medium text-dark dark:text-slate-200">
          Current Hostel
        </label>
        <select
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1.5fr] xl:grid-cols-[420px_1fr]">
        ...
        <section className="space-y-6">
          <div className="rounded-2xl bg-white dark:bg-slate-900 p-4 sm:p-5 md:p-6 shadow-sm dark:shadow-md border border-slate-100 dark:border-slate-800">
            <h2 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4 text-dark dark:text-white">
              Create Room
            </h2>
              ...
              <div className="grid gap-3 sm:gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      roomFormik.setFieldValue("room_number", e.target.value)
                    }
                    style={{ fontSize: "16px" }}
                  />
                </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    Auto-updates based on room type, but you can manually change
                    it
                  </p>
                </div>
                ...
                <button
                  ...
                  disabled={
                    ...
                    !selectedHostelId ||
                    !roomFormik.values.room_number.trim()
                  }
                  type="submit"
                >
              ...
              <div className="grid gap-3 sm:gap-4">
                ...
                <button
                  ...
                  disabled={
                    ...
                    !selectedRoomId ||
                    !bedFormik.values.bed_number.trim()
                  }
                  type="submit"
                >
        ...
        <section className="rounded-2xl bg-white dark:bg-slate-900 p-4 sm:p-5 md:p-6 shadow-sm dark:shadow-md border border-slate-100 dark:border-slate-800">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4">
            <h2 className="text-lg sm:text-xl font-semibold text-dark dark:text-white">
              Rooms and Beds
            </h2>
          ...
          <div className="mt-3 sm:mt-4 space-y-3 sm:space-y-4">
            ...
            {roomsQuery.data?.map((room) => (
              <article
                key={room.id}
                className="rounded-xl border border-slate-200 dark:border-slate-700 p-3 sm:p-4 bg-slate-50 dark:bg-slate-800/50"
              >
                {editingRoom?.id === room.id ? (
                  <div className="grid gap-2 sm:gap-3">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          type="number"
                          value={editRoomForm.daily_rent ?? 0}
                          onChange={(e) =>
                            setEditRoomForm((c) => ({
                              ...c,
                              daily_rent: Number(e.target.value),
                            }))
                          }
                          style={{ fontSize: "16px" }}
                        />
                      ...
                      <div>
                        ...
                        <input
                          ...
                          type="number"
                          value={editRoomForm.monthly_rent ?? 0}
                          onChange={(e) =>
                            setEditRoomForm((c) => ({
                              ...c,
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        onClick={() =>
                          updateRoomMutation.mutate(
                            { roomId: room.id, payload: editRoomForm },
                      ...
                      <button
                        className="flex items-center gap-1 rounded-xl border border-slate-300 dark:border-slate-600 px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-dark dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700"
                  ...
                  <div className="flex flex-col sm:flex-row items-start justify-between gap-2 sm:gap-4">
                    <div className="min-w-0">
                      <h3 className="font-semibold text-sm sm:text-base text-dark dark:text-white">
                        Room {room.room_number} · {room.room_type}
                      </h3>
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2">
                      <button
                        className="rounded-xl border bg-accent dark:bg-amber-700 border-slate-300 dark:border-amber-600 px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-dark dark:text-white hover:bg-accent/80 dark:hover:bg-amber-600"
                      ...
                      <button
                        className="rounded-xl border bg-primary dark:bg-primary/80 border-slate-300 dark:border-primary/50 px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-white hover:bg-primary/90 dark:hover:bg-primary"
                        onClick={() =>
                          setSelectedRoomId(
                    ...
                    {bedsQuery.data?.length ? (
                      <div className="grid gap-2 sm:gap-3 sm:grid-cols-2">
                        {bedsQuery.data.map((bed) => (
                          <div
                            key={bed.id}
                            className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-2 sm:p-3"
                          >
                            {editingBed?.id === bed.id ? (
                              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                                <select
                                  className="flex-1 rounded-lg border border-slate-300 dark:border-slate-600 px-2 py-1 text-xs sm:text-sm bg-white dark:bg-slate-700 text-dark dark:text-slate-200"
                                  value={editBedStatus}
                                  onChange={(e) =>
                                    setEditBedStatus(e.target.value)
                                  }
                                  style={{ fontSize: "16px" }}
                                >
                                  <option value="available">Available</option>
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    updateBedMutation.mutate(
                                      {
                                        bedId: bed.id,
                                        payload: { status: editBedStatus },
                                      },
                                      { onSuccess: () => setEditingBed(null) },
                                    )
                                  }
                                  type="button"
                                >
                                ...
                                <button
                                  className="rounded-lg border border-slate-300 dark:border-slate-600 p-1 sm:p-1.5 text-dark dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700"
                              ...
                              <div className="flex items-center justify-between">
                                <div>
                                  <p className="font-medium text-xs sm:text-sm text-dark dark:text-white">
                                    {bed.bed_number}
                                  </p>
                                ...
                                <button
                                  className="rounded-lg border border-slate-200 dark:border-slate-700 p-1 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400"
