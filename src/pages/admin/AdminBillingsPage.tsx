...
export function AdminBillingsPage() {
  ...
  const { data: subscription, isLoading: subLoading } = useAdminSubscription(
    userId,
    hostelIds,
    activeHostelId,
  );
  ...
  const { data: history = [], isLoading: historyLoading } =
    useAdminBillingHistory(userId, hostelIds, activeHostelId);

  const checkoutMutation = useCreateAdminCheckout(
    userId,
    hostelIds,
    activeHostelId,
  );
  const selectPlanMutation = useSelectAdminPlan(
    userId,
    hostelIds,
    activeHostelId,
  );
  ...
  const totalAmount = +(baseAmount + cgst + sgst).toFixed(2);
    ...
    <div className="max-w-6xl mx-auto space-y-6 text-slate-800">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-slate-500">
          Manage your plan, payment, and invoices.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="card p-6 border border-slate-200">
          ...
          <p className="text-2xl font-bold text-dark">
            {subscription?.last_payment_date || "N/A"}
          </p>
          <p className="text-xs text-slate-500 mt-1">
            {subscription?.last_payment_status || "N/A"}
          </p>
        </div>
      </div>
        ...
        <div className="xl:col-span-1 card p-6 border border-slate-200">
          ...
          <div className="space-y-3 border-b border-slate-100 pb-4 mb-4">
            ...
            <div className="flex justify-between text-[13px]">
              ...
              <span className="text-slate-600">₹{cgst.toFixed(2)}</span>
            </div>

            <div className="flex justify-between text-[13px]">
              <span className="text-slate-500">SGST (9%)</span>

              <span className="text-slate-600">₹{sgst.toFixed(2)}</span>
            </div>
          </div>
          ...
          <div className="mt-4 space-y-2">
            <p className="text-[11px] text-slate-400 leading-relaxed">
              GST split into CGST and SGST for intra-state billing. Inter-state
              customers see IGST instead.
            </p>

            <p className="text-[11px] text-slate-400">
              Payment verification refreshes your plan and invoice list
              automatically.
            </p>
          </div>
        </div>
      ...
      <div className="card border border-slate-200">
        <div className="p-6 border-b border-slate-100">
          ...
          <p className="text-[13px] text-slate-500">
            Recent payments and downloadable invoices.
          </p>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {history.length === 0 ? (
                <tr>
                  <td
                    colSpan={7}
                    className="px-6 py-8 text-center text-[13px] text-slate-500"
                  >
                ...
                history.map((h, i) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 font-bold text-dark text-[13px]">
                      {h.plan_name}
                    </td>
