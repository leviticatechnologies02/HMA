...
import { UtensilsCrossed, Leaf, Drumstick, Clock } from "lucide-react";
import { useSupervisorMessMenu } from "../../hooks/useSupervisorNotices";
import { useAuthStore } from "../../store/authStore";
import { FOOD_IMAGES } from "../../utils/images";
...
export function SupervisorMessMenuPage() {
  const userId = useAuthStore((s) => s.userId);
  const menuQ = useSupervisorMessMenu(userId);
  ...
  if (!userId) return <div className="p-8 text-slate-500 dark:text-slate-400">Please login as supervisor.</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      {!menuQ.isLoading && menuItems.length === 0 && (
      ...
      {!menuQ.isLoading && !menuQ.isError && menuItems.length > 0 && (
        <>
          {/* Active week banner */}
          {activeWeekStart && (
            <div className="bg-secondary/10 dark:bg-secondary/20 border border-secondary/20 dark:border-secondary/30 rounded-2xl p-4 flex items-center gap-3">
              <UtensilsCrossed className="w-5 h-5 text-secondary shrink-0" />
              <div>
                <p className="font-semibold text-dark dark:text-white">
                  Current Week: {formatDate(activeWeekStart)}
                </p>
                <p className="text-sm text-slate-500 dark:text-slate-400">Menu is active for this week</p>
              </div>
            ...
            {DAYS_OF_WEEK.map(day => {
              ...
              return (
                <button
                  ...
                  className={`px-5 py-2.5 rounded-xl text-sm font-medium whitespace-nowrap transition-all flex items-center gap-2 ${selectedDay === day
              ...
              return (
                <div key={mealType} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden card-hover flex flex-col h-full">
                  <div className="h-32 relative overflow-hidden shrink-0">
                    <img src={FOOD_IMAGES[mealType as keyof typeof FOOD_IMAGES]} alt={mealType} className="w-full h-full object-cover transition-transform hover:scale-105 duration-500" loading="lazy" />
                      ...
                      <>
                        ...
                        <div className="mt-auto pt-3 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between">
                          ...
                          {mealData?.special_note && (
                            <span className="text-[10px] bg-secondary/10 dark:bg-secondary/20 text-secondary dark:text-secondary/80 border border-secondary/20 dark:border-secondary/30 px-2 py-0.5 rounded-full font-medium truncate max-w-[100px]" title={mealData.special_note || undefined}>
                              {mealData.special_note}
                            </span>
                          )}
                        </div>
